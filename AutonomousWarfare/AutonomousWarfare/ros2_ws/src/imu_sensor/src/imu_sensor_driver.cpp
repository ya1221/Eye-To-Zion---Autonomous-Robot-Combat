...
{
  if (icm_fd_ >= 0) close(icm_fd_);
  if (mag_fd_ >= 0) close(mag_fd_);
}
...
bool ImuDriverNode::open_i2c()
{
  ...
  icm_fd_ = open(i2c_path.c_str(), O_RDWR);
  if (icm_fd_ < 0) {
    RCLCPP_ERROR(this->get_logger(), "Cannot open I2C bus at %s: %s",
      i2c_path.c_str(), strerror(errno));
    return false;
  }
  if (ioctl(icm_fd_, I2C_SLAVE, i2c_address_) < 0) {
    RCLCPP_ERROR(this->get_logger(), "Cannot set I2C address 0x%02X: %s",
      i2c_address_, strerror(errno));
    return false;
  }

  // Open a separate file descriptor for the magnetometer (AK09916 at 0x0C).
  // This avoids switching I2C_SLAVE address back and forth on the same fd.
  if (enable_magnetometer_) {
    mag_fd_ = open(i2c_path.c_str(), O_RDWR);
    if (mag_fd_ < 0) {
      RCLCPP_ERROR(this->get_logger(), "Cannot open I2C bus for magnetometer: %s",
        strerror(errno));
      return false;
    }
    if (ioctl(mag_fd_, I2C_SLAVE, AK09916_ADDR) < 0) {
      RCLCPP_ERROR(this->get_logger(), "Cannot set I2C address 0x%02X for magnetometer: %s",
        AK09916_ADDR, strerror(errno));
      return false;
    }
  }
...
bool ImuDriverNode::init_icm20948()
{
  ...
  if (!write_register(icm_fd_, bank0::PWR_MGMT_1, PWR_MGMT_1_DEVICE_RESET)) {
    RCLCPP_ERROR(this->get_logger(), "Failed to reset ICM-20948.");
    return false;
  }
  std::this_thread::sleep_for(std::chrono::milliseconds(100));

  // 2) Wake up, auto-select clock
  if (!write_register(icm_fd_, bank0::PWR_MGMT_1, PWR_MGMT_1_CLKSEL_AUTO)) {
  ...
  // 3) Verify WHO_AM_I
  uint8_t who_am_i = 0;
  if (!read_register(icm_fd_, bank0::WHO_AM_I, who_am_i)) {
    RCLCPP_ERROR(this->get_logger(), "Failed to read WHO_AM_I.");
    return false;
  }
  ...
  if (!write_register(icm_fd_, bank0::PWR_MGMT_2, PWR_MGMT_2_ALL_ENABLED)) return false;
  ...
  if (!write_register(icm_fd_, bank2::GYRO_CONFIG_1, gyro_config)) {
    RCLCPP_ERROR(this->get_logger(), "Failed to configure gyroscope.");
    return false;
  }
  // Gyro sample rate divider = 0 → ODR = 1125 / (1 + 0) = 1125 Hz (internal)
  if (!write_register(icm_fd_, bank2::GYRO_SMPLRT_DIV, 0x00)) return false;

  // 6) Configure accel (Bank 2)
  uint8_t accel_config = get_accel_fs_bits() | ACCEL_DLPF_ENABLE | ACCEL_DLPF_CFG_3;
  if (!write_register(icm_fd_, bank2::ACCEL_CONFIG, accel_config)) {
    RCLCPP_ERROR(this->get_logger(), "Failed to configure accelerometer.");
    return false;
  }
  // Accel sample rate divider = 0 → ODR = 1125 / (1 + 0) = 1125 Hz
  if (!write_register(icm_fd_, bank2::ACCEL_SMPLRT_DIV_1, 0x00)) return false;
  if (!write_register(icm_fd_, bank2::ACCEL_SMPLRT_DIV_2, 0x00)) return false;

  // 7) Enable I2C bypass mode so we can directly access the AK09916 magnetometer
  //    (Must disable I2C master first)
  if (!select_bank(0)) return false;
  if (!write_register(icm_fd_, bank0::USER_CTRL, 0x00)) return false;  // Disable I2C master
  if (!write_register(icm_fd_, bank0::INT_PIN_CFG, INT_PIN_CFG_BYPASS_EN)) return false;
...
// ═══════════════════════════════════════════════════════════════════════════════
// AK09916 Magnetometer Initialization (via I2C bypass)
// ═══════════════════════════════════════════════════════════════════════════════

bool ImuDriverNode::init_ak09916()
{
  // 1) Soft reset
  if (!write_register(mag_fd_, ak09916::CNTL3, AK09916_SOFT_RESET)) {
  ...
  // 2) Verify device ID
  uint8_t device_id = 0;
  if (!read_register(mag_fd_, ak09916::WIA2, device_id)) {
  ...
  if (device_id != AK09916_DEVICE_ID) {
    RCLCPP_ERROR(this->get_logger(),
      ...
      "Check I2C bypass and magnetometer wiring.",
  ...
  if (!write_register(mag_fd_, ak09916::CNTL2, AK09916_MODE_CONT_100HZ)) {
    RCLCPP_ERROR(this->get_logger(), "Failed to set AK09916 continuous mode.");
    return false;
  }
  std::this_thread::sleep_for(std::chrono::milliseconds(10));

  RCLCPP_INFO(this->get_logger(), "AK09916 configured: continuous 100 Hz mode.");
  return true;
}
...
void ImuDriverNode::read_and_publish()
{
  ...
  if (!read_accel_gyro(ax, ay, az, gx, gy, gz)) {
    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
      "Failed to read accel/gyro data.");
    return;
  }
  ...
  // Orientation is NOT computed here — set covariance[0] = -1 to indicate unknown.
  // imu_filter_madgwick will fill this in downstream.
  ...
  // ─── Read and publish temperature (optional) ───────────────────────────
  if (publish_temperature_) {
    double temp_c;
    if (read_temperature(temp_c)) {
      ...
      temp_msg.variance        = 1.0;  // Rough estimate
...
bool ImuDriverNode::write_register(int fd, uint8_t reg, uint8_t value)
{
...
bool ImuDriverNode::read_register(int fd, uint8_t reg, uint8_t & value)
{
  if (write(fd, &reg, 1) != 1) {
    RCLCPP_ERROR(this->get_logger(), "I2C write (set register) failed: reg=0x%02X: %s",
      reg, strerror(errno));
    return false;
  }
...
bool ImuDriverNode::read_registers(int fd, uint8_t start_reg, uint8_t * buffer, size_t length)
{
...
bool ImuDriverNode::select_bank(uint8_t bank)
{
  ...
  if (!write_register(icm_fd_, REG_BANK_SEL, bank_val)) {
  ...
  double & gx, double & gy, double & gz)
{
  ...
  uint8_t buf[12];
  if (!read_registers(icm_fd_, bank0::ACCEL_XOUT_H, buf, 12)) {
    return false;
  }
...
bool ImuDriverNode::read_magnetometer(double & mx, double & my, double & mz)
{
  // Check data ready
  uint8_t st1 = 0;
  if (!read_register(mag_fd_, ak09916::ST1, st1)) return false;
  if (!(st1 & 0x01)) {
    return false;  // Data not ready yet — skip this cycle
  }

  // Read 8 bytes: HXL(0x11) through ST2(0x18)
  // Layout: HXL, HXH, HYL, HYH, HZL, HZH, dummy(0x17), ST2(0x18)
  uint8_t buf[8];
  if (!read_registers(mag_fd_, ak09916::HXL, buf, 8)) return false;
  ...
  int16_t raw_mx = static_cast<int16_t>(buf[0] | (buf[1] << 8));
  int16_t raw_my = static_cast<int16_t>(buf[2] | (buf[3] << 8));
  int16_t raw_mz = static_cast<int16_t>(buf[4] | (buf[5] << 8));
...
bool ImuDriverNode::read_temperature(double & temp_c)
{
  ...
  uint8_t buf[2];
  if (!read_registers(icm_fd_, bank0::TEMP_OUT_H, buf, 2)) return false;
