...
import numpy as np
import time
import queue # הוספנו את ספריית התורים המובנית של פייתון
...
# שים לב: העברנו את msg_queue במקום את p (ה-pubsub של רדיס)
def get_latest_world_state(msg_queue, current_enemies_list, current_teammates_dict, static_obstacles, ros_node=None, aruco_pub=None):
    global danger_dict
    
    enemies_list, teammates_dict = check_zenoh_updates(msg_queue, current_enemies_list, current_teammates_dict, ros_node, aruco_pub)
...
def check_zenoh_updates(msg_queue, enemies_list, teammates_dict, ros_node=None, aruco_pub=None):
    current_time = time.time()
    
    # במקום לשאול את רדיס, אנחנו פשוט מרוקנים את התור שהתמלא ברקע ע"י Zenoh
