...
class AIInferenceNode(Node):
    ...
    def metadata_callback(self, msg):
        ...
        # CRITICAL ALERT: Print IMMEDIATELY if performance drops dangerously
        if fps < 7.0:
            self.get_logger().warn(f"CRITICAL: FPS dropped to {fps:.2f}!")
        # HEARTBEAT: Print normally once every 10 frames just for status
        ...
        # Run inference with reduced resolution for NCNN
